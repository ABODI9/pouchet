...
export class FeaturedService {
  ...
  /** اشترك فيها لو تحب تعيد التحميل عند أي تعديل (السلايدر يستخدمها) */
  ...
  // رفع عدّة صور في طلب واحد: images[]
  createMany(
    files: File[],
    opts?: { order?: number; active?: boolean; productId?: string | null; caption?: string; openInNewTab?: boolean; }
    ...
    if (opts?.openInNewTab != null) fd.append('openInNewTab', String(!!opts.openInNewTab));

    return this.http.post<FeaturedItem[]>(`${this.base}/${this.path}`, fd)
      .pipe(tap(() => this._updated.next()));
  }

  update(id: string, body: FormData): Observable<FeaturedItem> {
    return this.http.put<FeaturedItem>(`${this.base}/${this.path}/${id}`, body)
      .pipe(tap(() => this._updated.next()));
  }

  remove(id: string) {
    return this.http.delete(`${this.base}/${this.path}/${id}`)
      .pipe(tap(() => this._updated.next()));
  }
}
