...
import { JwtPayload, LoginRequest, LoginResponse, RegisterRequest, Role } from '../models/auth.model';
...
export class AuthService {
  ...
  /** الدور كمجرى مستقل */
  readonly role$: Observable<Role | null> = this.user$.pipe(map(u => u?.role ?? null));

  /** خصائص مريحة للمكونات */
  get token(): string | null { return this._token$.value; }
  get user(): JwtPayload | null { return this.decode(this.token); }
  get role(): Role | null { return this.user?.role ?? null; }
  get isAuthenticated(): boolean { return !!this.token && !this.isExpired(this.token); }
  ...
  register(dto: RegisterRequest): Observable<void> {
    return this.http.post<void>(`${API}/auth/register`, dto).pipe(map(() => void 0));
  }

  me() {
    return this.http.get<{ userId: string; email: string; role: Role }>(`${API}/auth/me`);
  }
